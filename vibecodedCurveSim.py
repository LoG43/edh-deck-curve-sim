...
Pass 14 (the deckbuilder-facing statistics summary is now a real,
shippable feature -- `--summary`; previously it only existed as a
one-off script run from outside this repository entirely):
...
def generate_statistics_summary(decklist_path: Path, commander_names: list, library_counts: dict,
    ...
    lines.append("")
    lines.append(f"      ON CURVE (turn == its own cmc):  {avg_or_dash(0):>6}")
    lines.append(f"      by cmc+1:                        {avg_or_dash(1):>6}")
    lines.append(f"      by cmc+2:                        {avg_or_dash(2):>6}")
    lines.append(f"      by cmc+3:                        {avg_or_dash(3):>6}")
    lines.append("")
...
def main(argv=None) -> int:
    ...
    if args.summary:
        summary_text = generate_statistics_summary(
            ...
            results, args.simulations, args.max_turns,
        )
